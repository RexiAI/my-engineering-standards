...
`opencode/mimo-v2.5-free` model and the OpenCode provider block are the
...
`opencode/*` model ids used across this repo. It is wired in two places:

- **Provider config** — `opencode.json` carries a `provider.opencode` block:
  base URL `https://opencode.ai/zen/v1`, auth env var `OPENCODE_API_KEY`, and
...
spec 024) carries a literal `model: opencode/mimo-v2.5-free` in its frontmatter —
