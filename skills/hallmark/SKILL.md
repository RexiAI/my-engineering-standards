...
license: Proprietary. License at repo root.
version: 1.2.0
---
...
- [`upgrade-techniques.md`](references/upgrade-techniques.md) — load during the Design flow (default verb) and `hallmark redesign`. Contains high-impact techniques (variable font animation, broken grid, parallax stacks, spring physics, glassmorphism, spotlight borders, grain overlays). Pick 1–3 per build; the selection guide at the end of the file explains when each technique fits.
- [`code-quality.md`](references/code-quality.md) — load only when `hallmark audit` targets an **existing** codebase. Covers semantic HTML, CSS hygiene, `<head>` completeness, JS/build quality. **Do not load for greenfield builds or component-scope audits.**
- [`strategic-omissions.md`](references/strategic-omissions.md) — load only when `hallmark audit` targets an **existing** codebase. Covers back navigation, active nav indicator, form validation, dead links, legal links, loading/empty/error states, 404 page. **Do not load for greenfield builds or component-scope audits.**
