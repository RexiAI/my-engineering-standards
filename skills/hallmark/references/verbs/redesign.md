...
**Upgrade techniques.** Always load [`../upgrade-techniques.md`](../upgrade-techniques.md) at the start of a single-page redesign. After diagnosing the existing page's tells, select **1–3 techniques** from that file that amplify the chosen macrostructure. State the picks before writing code: *"Upgrade techniques: staggered entry (amplifies Stat-Led rhythm) · spotlight borders (adds tactile depth to feature cards)."* Skip techniques that compete with the macrostructure or that lack clean `prefers-reduced-motion` fallbacks.
