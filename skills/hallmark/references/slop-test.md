# Slop test — 66 gates + pre-emit self-critique
...
## Surface and layout tells

These gates cover the new tells added from the redesign-skill audit. They apply to both new builds and existing-project redesigns.

58. **Random dark/light section inversion.** Does an otherwise light-mode page contain one or more dark-background sections (`background` OKLCH lightness < 40%) inserted mid-page with no system token rationale? Auto-fail. A dark section inversion signals a page assembled from disconnected components. Fix: use `--color-paper-3` (a slightly darker band of the same hue family) instead of near-black. If a genuine full-bleed dark block is needed, define it as a named system token (`--color-section-dark`) and ensure all descendant text tokens flip to `--color-paper`. *(See [anti-patterns.md § Random dark section inversion](anti-patterns.md).)*

59. **Dead links.** Does any interactive element — `<a>`, `<button>`, a styled `<div role="button">` — have `href="#"`, no `href`, `onclick="return false"`, or a `disabled` attribute while appearing visually active (not styled as disabled)? Auto-fail. A clickable element that does nothing is a trust violation. Fix: link to the real destination, implement the real handler, or style the element as disabled with `opacity: 0.55`, `cursor: not-allowed`, and the native `disabled` attribute. *(See [strategic-omissions.md § Dead links](../strategic-omissions.md).)*

60. **`height: 100vh` instead of `min-height: 100dvh`.** Does any full-viewport section declare `height: 100vh` or `min-height: 100vh`? Fail. On iOS Safari, `100vh` includes the toolbar height and overflows the visible area. Fix: always use `min-height: 100dvh`. *(See [anti-patterns.md § height: 100vh](anti-patterns.md).)*

61. **Buttons not bottom-aligned in card groups.** In any card row where cards have variable-length content, do the CTA buttons sit at different vertical positions? Fail. Fix: make each card `display: flex; flex-direction: column` with the content area above the button set to `flex: 1`. *(See [anti-patterns.md § Buttons not bottom-aligned in card groups](anti-patterns.md).)*

62. **Div soup.** Does the page's primary structure lack semantic HTML — no `<nav>`, `<main>`, `<article>`, `<aside>`, `<section>` elements, relying entirely on `<div>`? Fail for existing-project audits and `hallmark redesign` outputs. Pass for greenfield builds during initial scaffold if the model has not yet had a chance to add semantics. Fix: replace structural divs with their semantic equivalents. *(See [code-quality.md § Div soup](../code-quality.md).)*

63. **Missing alt text on meaningful images.** Does any `<img>` carrying meaningful content (product screenshot, portrait, diagram) have `alt=""` or `alt="image"`? Fail. Fix: describe the content and function. Empty `alt=""` is correct only for purely decorative images. *(See [code-quality.md § Missing alt text](../code-quality.md).)*

64. **Orphaned words on display text.** Does any heading element (`h1`–`h3`) or hero display text lack `text-wrap: balance`? Fail when any such element, rendered at its in-page font size, produces a final line of a single word. Fix: add `text-wrap: balance` to all heading elements globally. For browsers without support, the property is safely ignored; no polyfill needed. *(See [anti-patterns.md § Orphaned words](anti-patterns.md).)*

65. **Oversaturated accent.** Does the `--color-accent` OKLCH value have chroma `C > 0.22` for warm hues (H 0–80°, 300–360°) or `C > 0.20` for cool hues (H 180–280°)? Fail. High-chroma accents read as "maxed out the colour picker." Fix: pull C down to the threshold range. Verify gate 41 (button text ≈ button fill) after adjusting. *(See [anti-patterns.md § Oversaturated accent](anti-patterns.md).)*

The CSS stamp at Step 6 records these results: `· surface: pass (58–60) · layout: pass (61) · semantic: pass (62–63) · typography: pass (64) · colour: pass (65)`. Fix any failure before shipping.
