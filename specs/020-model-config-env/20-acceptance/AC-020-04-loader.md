# AC-020-04: `scripts/load-model-env.sh` exports every model var with env > local-file > example precedence
...
When `scripts/load-model-env.sh` runs against that fixture (fixture root passed as the positional argument)
...
When `scripts/load-model-env.sh` runs against that fixture
...
Given `scripts/load-model-env.sh` in the repo
When it is sourced from a different working directory than the repo root, with no positional argument, and `config/model.local.env` present in the repo
Then it still exports the vars (config paths resolve to the repo root by default, documented in the script header)
