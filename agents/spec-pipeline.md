...
- **`/build <slug>`**: confirm `10-tasks.md` and `20-acceptance/` exist for that
  ...
  `spec-refactorer`, then `spec-verifier`, then `spec-architect`, in that order, each waiting for
  the previous to finish. Surface each subagent's end-of-turn summary as you go. If
  the Verifier's verdict is FAIL, stop and relay its report — do not run `spec-architect`
  anyway, and do not attempt to fix the failure yourself. If any stage reports it
  cannot proceed (ambiguous scenario, failing gate), stop and relay exactly what it
  said — do not paper over it or attempt the stage yourself.
